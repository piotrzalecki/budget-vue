<template>
  <div class="container">
    <div class="row">
      ...
      <form-tag
        v-if="ready"
        @transactionEditEvent="submitHandler"
        name="transactionFrom"
        event="transactionEditEvent"
      >
        <text-input
        ...
        <div class="mb-3">
          ...
          >
            <option v-for="b in budgets" :value="b.id" :key="b.id">
              {{ b.name }}
            </option>
          </select>
        </div>
        ...
        <div class="mb-3">
          ...
          >
            <option v-for="t in trs" :value="t.id" :key="t.id">
              {{ t.name }}
            </option>
          </select>
        </div>
        ...
        <div class="mb-3">
          <label for="transaction_starts" class="form-label">Starts At</label
          ><br />
  ...
  setup(props, cxt) {
    ...
    function submitHandler() {
      const payload = {
        ...
        active: transaction.value.active === true, // Explicitly compare to true
      ...
      fetch(
        ...
        Security.requestOptions(payload, "PUT")
      )
    ...
    function confirmDelete(id) {
      notie.confirm({
        ...
        submitCallback: () => {
          ...
          fetch(
            `${process.env.VUE_APP_API_URL}/admin/dashboard/transactions`,
            Security.requestOptions(payload, "DELETE")
          )
            .then((response) => response.json())
            .then((data) => {
              if (data.error) {
    ...
    onBeforeMount(() => {
      ...
      if (route.params.transactionId > 0) {
        fetch(
          `${process.env.VUE_APP_API_URL}/admin/dashboard/transactions/${route.params.transactionId}`,
          ...
          .catch((data) => {
            ...
            cxt.emit(
              "error",
              `Transaction of id ${route.params.transactionId} can't be obtained form backend.`
            );
          });
      }
      //fetch budgets
      ...
      //fetch tags
      fetch(
        `${process.env.VUE_APP_API_URL}/admin/dashboard/tags`,
        Security.requestOptions("")
      )
        .then((response) => response.json())
        .then((data) => {
      ...
      fetch(
        `${process.env.VUE_APP_API_URL}/admin/dashboard/transactions-recurrences`,
        Security.requestOptions("")
      )
        .then((response) => response.json())
        .then((data) => {
